...
async def _store(user_id: UUID, filename: str, size_bytes: int, pages: list[tuple[int | None, str]]) -> Document:
    ...
    async with tenant_session(user_id) as session:
        document = Document(user_id=user_id, filename=filename, chunk_count=len(chunks), size_bytes=size_bytes)
...
async def ingest_from_path(user_id: UUID, filename: str, path: str) -> Document:
