...
The GitLab corpus comparison runs in GitHub Actions (`.github/workflows/eval.yml`) every six hours until it is finished: each run starts its own API on the runner against a throwaway Postgres, with the evaluation keys from the repository secrets `EVAL_GROQ_API_KEY` and `EVAL_OPENROUTER_API_KEY` (never the production ones), and continues where the previous one stopped. Progress lives in the production table `eval_runs`, apart from user data: the runner pulls it from `GET /eval/runs/{name}` and pushes it after every question with `PUT /eval/runs/{name}`, both guarded by `EVAL_WRITE_TOKEN` (the API only keeps its SHA-256, `EVAL_WRITE_TOKEN_SHA256`). `GET /eval/status` is public and returns the averages and the progress without answers or sources; the landing page reads it. `app/eval/run_eval.py` pushes the same way locally when `EVAL_SYNC_URL` and `EVAL_WRITE_TOKEN` are set, and exits with code 75 when a daily quota ends the run.
