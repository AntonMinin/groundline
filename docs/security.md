...
- `POST /auth/request-otp` requires `accepted_terms: true` and answers `422` without it, so the consent box on the login screen cannot be skipped by a client that does not render it.
...
- **No Content-Security-Policy.** `frontend/vercel.json` and `landing/vercel.json` set `X-Frame-Options: DENY`, `frame-ancestors 'none'`, `nosniff`, `Referrer-Policy` and `Permissions-Policy`, but not a script or connect policy: React inline styles and the configurable API origin would make one that is both correct and useful larger than it is worth here.
